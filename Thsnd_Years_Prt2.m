...
while (true)
 command = input('Press [0] to begin/run again\n      [9] to quit \n      [1] to Fade In \n      [2] to Fade Out \n      [3] to Pitch Up \n      [4] to Pitch Down \n      [5] to Plot \n      [6] to Normalize\nEnter Choice: ');
 
 if command == 0
     signal = original_signal;
     fprintf('Running Original... \n');
 elseif command == 9 % exit and save wav file
     x = max(signal);
     signal = 0.9*signal/abs(x);
     audiowrite('Output_Part2.wav', signal, fs);
     break;
 elseif command == 1 %fade_in
     signal = original_signal;
     signal = signal .* z1;
 elseif command == 2 %fade_out
     signal = original_signal;
     signal = signal .* z2;
 elseif command == 3 %pitch up
     signal = original_signal;
     signal = (2 * signal .^ 2) - 1;
 elseif command == 4 %pitch down
     signal = original_signal;
     signal = sqrt((signal + 1) / 2);
 elseif command == 5 %plot
     figure;
     plot(signal);
 elseif command == 6 %normalize
     signal = original_signal;
     fprintf('Playing Normalized... \n');
 end
 
 % Normalizing to sound good
 x = max(signal);
 signal = 0.9*signal/abs(x);
